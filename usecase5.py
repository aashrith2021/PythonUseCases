...
#list of dicts contaning data of states
...
#printing the data
for state_data in indian_state_profiles:
    print([cat for cat in state_data.values()])

#setting the fields
fields=['name','population','area rank']

#using DictWriter, writeheader and writerows to fill the csv
with open('state.csv','w') as fileObj:
    stateobj=csv.DictWriter(fileObj,fieldnames=fields)
    stateobj.writeheader()
    stateobj.writerows(indian_state_profiles)
